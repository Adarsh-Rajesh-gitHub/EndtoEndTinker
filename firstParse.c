...
uint64_t pc = 0x2000; 
uint64_t pcData = 0x10000;

//for tinker file header
uint64_t fileType = 0;
uint64_t codeStart = 0x2000;
uint64_t codeSize;
...
int main(int argc, char* args[]) {
    ...
    while(fgets(line, sizeof line, fp)) {
        ...
        if(c == ';') {
            ...
            if(mode == 1) {
                //label resolution flush by adding to hashmap 
                for(int i = 0; i < curLabels->numElements; i++) {
                    if(!label_add(curLabels->entries[i],hM,labels_seen, pc)){
                ...
                clearList(curLabels);
                //back to addressing current line code
                printf("code line\n");
            ...
            //label resolution flush by adding to hashmap 
            for(int i = 0; i < curLabels->numElements; i++) {
                if(!label_add(curLabels->entries[i],hM,labels_seen, pcData)){
                    fprintf(stderr,"error: invalid or duplicate label\n");
                    fclose(fp);
                    return 1;
                }
            ...
            clearList(curLabels);
            //back to addressing current line data
            uint64_t v=0;
    ...
    if(curLabels->numElements != 0) {
        //labels remaining at end with no code or data before
        if(mode != 1 && mode != -1) {
            fprintf(stderr, "remaining labels at end w. out any code or data before");
            return 1;
        }
        uint64_t curPc = mode == 1 ? pc : pcData;
        for(int i = 0; i < curLabels->numElements; i++) {
            if(!label_add(curLabels->entries[i],hM,labels_seen, curPc)){
                fprintf(stderr,"error: invalid or duplicate label\n");
                fclose(fp);
                return 1;
            }
    ...
    //For prog5 no writing to intermediate file
    //write to the intermediate file
    char tmpi[512];
    snprintf(tmpi,sizeof(tmpi),"%s.tmp","intermediate");
...
char tmpb[512];
snprintf(tmpb,sizeof(tmpb),"%s.tmp",args[2]);

FILE *out=fopen(tmpb,"wb");
if(!out){ fprintf(stderr,"can't open out\n"); remove(tmpi); return 1; }

//write in struct headers
codeSize = pc - 0x2000;
dataSize = pcData - 0x10000;
fwrite(&fileType, 8, 1, out);
fwrite(&codeStart, 8, 1, out);
fwrite(&codeSize, 8, 1, out);
fwrite(&dataStart, 8, 1, out);
fwrite(&dataSize, 8, 1, out);

//main loop to write in binary file
for(int i = 0; i < intermediate->numElements; i++) {
